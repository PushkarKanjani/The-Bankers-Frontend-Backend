...
interface MaxTableProps {
  max: number[][];
  onMaxChange: (processIndex: number, resourceIndex: number, value: number) => void;
}

export const MaxTable: React.FC<MaxTableProps> = ({ max, onMaxChange }) => {
  const handleInputChange = (pIdx: number, rIdx: number, value: string) => {
    const num = parseInt(value) || 0;
    if (num >= 0) {
      onMaxChange(pIdx, rIdx, num);
    }
  };
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
      <h2 className="text-xl font-bold text-orange-400 mb-4">
        Client Credit Lines (Maximum Need)
      </h2>
          ...
          <tbody>
            {['P0', 'P1', 'P2', 'P3', 'P4'].map((processId, pIdx) => (
              <tr key={processId} className="hover:bg-gray-750">
                <td className="px-4 py-3 text-orange-400 font-mono font-semibold border border-gray-700">
                  {processId}
                </td>
                {[0, 1, 2].map((rIdx) => (
                  <td key={rIdx} className="px-2 py-2 border border-gray-700">
                    <input
                      ...
                      className="w-full px-2 py-1 bg-gray-900 border border-gray-600 rounded text-orange-300 font-mono text-center focus:outline-none focus:border-orange-500"
                    />
                  </td>
                ))}
              </tr>
